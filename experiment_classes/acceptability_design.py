...
class AcceptabilityDesign:
        ...
        if sent_key in self.scores:
            ...
            if self.current_sentence < self.sentences_quantity:
                for child in self.main_box.children:
                    ...
                    if text == self.sentences[self.current_sentence - 1]:
    ...
    def display_sentence(self, *args):
        ...
        saved = saved[1:]
        sent = Label(text=self.sentences[self.current_sentence-1], font_size="30",
                     color=(0, 0, 0, 1)
                    )
        sent.text_size = sent.width + 600, sent.height
